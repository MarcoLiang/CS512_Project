...
class MetaPathGenerator:
    ...
    def write_pattern_path_reverse_path(self, meta_path, file, inTest):
        '''
    ...
    def random_walk(self, dir_out_train):
        ...
        for author_id in self.id_author.keys():
            ...
            while path_cnt < self.path_per_author:
                ...
                if entity_type == self.author_type and (node.id in self.a_id_train) and node.step > 0:
                    inTestSet = author_id in self.a_id_test
                    if self.reverse_path:
                        self.write_pattern_path_reverse_path(meta_path[:node.step + 1], DBLP_train, inTestSet)
                    else:
                ...
                if node.step == 0: # generate neighbors form the start author
                    p_id =  np.random.choice(self.author_paper[node.id])
                    stack.push(Node(p_id, node.step + 1, 1))

                elif entity_type == self.conf_type and not node.id in meta_path[:node.step]:
                    p_id = np.random.choice(self.conf_paper[node.id])
                    stack.push(Node(p_id, node.step + 1, 1))

                elif entity_type == self.paper_type:
                    choice_type = np.random.choice([self.author_type, self.conf_type, self.paper_type], size=1, p=self.random_prob)

                    # paper to paper relation, stop if more than 3 consective papers
                    if choice_type == self.paper_type and node.paper_num < 3 :

                        if node.id in self.paper_to_paper and not node.id in meta_path[: node.step]:
                            p_id = np.random.choice(self.paper_to_paper[node.id])
                            stack.push(Node(p_id, node.step + 1, node.paper_num + 1))

                        if node.id in self.paper_from_paper and not node.id in meta_path[: node.step] : # some papers are not be cited
                            p_id2 = np.random.choice(self.paper_from_paper[node.id])
                            stack.push(Node(p_id2, node.step + 1, node.paper_num + 1))

                    # paper conference relation
                    if choice_type == self.conf_type and node.id in self.paper_conf and not node.id in meta_path[:node.step]:
                        conf_id = self.paper_conf[node.id]
                        stack.push(Node(conf_id, node.step + 1, 0))

                    # paper author relation
                    if choice_type == self.author_type and not node.id in meta_path[:node.step]:
                        a_id = np.random.choice(self.paper_author[node.id])
...
def main():
    ...
    # dir_input = "data/mini"
    r = 0.9
    random_prob = [1/3, 1/3, 1/3] # author; conf; paper
    print("#====================================================#")
    print("#            train : test = {0} : {1}                #".format(r, round(1-r, 1)))
    print("#====================================================#")
    dir_output = "data/random_walk/pattern_" + str(int(r * 100)) + "_" + str(int(np.ceil((1-r) * 100)))
    print("Start...")
